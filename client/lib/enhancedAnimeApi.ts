...
class EnhancedAnimeAPI {
  ...
  async searchAnimeEnhanced(query: string, limit: number = 20): Promise<EnhancedAnimeData[]> {
    try {
  ...
  async getAnimeFullDetails(animeId: number): Promise<{ anime: EnhancedAnimeData; episodes: AnimeEpisode[] }> {
    try {
  ...
  async searchAndGetFull(query: string): Promise<Array<{ anime: EnhancedAnimeData; episodes: AnimeEpisode[] }>> {
    try {
  ...
  convertToFullAnimeData(anime: EnhancedAnimeData, episodes: AnimeEpisode[] = []): any {
    ...
    return {
      ...
      banner: anime.images?.jpg?.large_image_url || anime.images?.webp?.large_image_url,
      ...
      description: anime.synopsis || `${anime.title} - Açıklama yakında eklenecek`,
      descriptionEn: anime.synopsis || `${anime.title} - Description coming soon`,
      status: status,
      category: category,
      ...
      episodeList: episodes.map(ep => ({
        ...
        descriptionEn: ep.synopsis || `Episode ${ep.episode_number} description`,
  ...
  async bulkImportAnime(titles: string[]): Promise<any[]> {
    ...
    for (const title of titles) {
      try {
        ...
        if (searchResults.length > 0) {
          const fullData = this.convertToFullAnimeData(searchResults[0].anime, searchResults[0].episodes);
  ...
  async enhanceImageQuality(animeData: any): Promise<any> {
    try {
      ...
      if (this.isLowQualityImage(animeData.poster)) {
        const alternativeImages = await this.getAlternativeImages(animeData.title);
