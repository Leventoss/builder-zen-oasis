...
const ANIME_RELATED_KEYWORDS = [
  'anime', 'manga', 'episode', 'bölüm', 'izle', 'watch', 'player', 'video',
  'site', 'hesap', 'account', 'giriş', 'login', 'kayıt', 'register',
  'premium', 'abonelik', 'subscription', 'destek', 'support', 'yardım', 'help',
  'sorun', 'problem', 'hata', 'error', 'çalışmıyor', 'not working',
  'one piece', 'naruto', 'black clover', 'demon slayer', 'attack on titan'
];

const OFF_TOPIC_KEYWORDS = [
  'kod', 'code', 'programming', 'python', 'javascript', 'html', 'css',
  'matematik', 'math', 'fizik', 'physics', 'kimya', 'chemistry',
  'tarih', 'history', 'coğrafya', 'geography', 'yemek', 'food', 'recipe'
];
...
function getTopicSpecificResponse(category: string, message: string, isPremium: boolean = false): string {
  const lowerMessage = message.toLowerCase();
...
Bu bilgileri verdikten sonra ekletme işlemini admin ekibimize ileteceğim.`;
    ...
    case 'anime-sorun':
      if (lowerMessage.includes('çalışmıyor') || lowerMessage.includes('sorun') || lowerMessage.includes('problem')) {
...
Spesifik bir sorunuz var mı?`;
  ...
  if (lowerMessage.includes('merhaba') || lowerMessage.includes('hello') || lowerMessage.includes('hi')) {
    const premiumGreeting = isPremium ? " Premium üyemizsiniz, size özel destek sağlayacağım!" : "";
...
export const handleAIChat: RequestHandler = async (req, res) => {
  try {
    const { message, category, sessionId, userId, isPremium }: ChatRequest = req.body;
